...
REPO_URL = "https://github.com/silvia13222/django_polls"
...
def clone(ctx): 
    ...
    if isinstance(ctx, Connection):  # para no hacer conexion en cada uno de los pasos
        conn = ctx
    else:
...
#conn.run(f"{VENV_PIP} install -r requirement.txt")
@task
def migrate(ctx):
...
def loaddata(ctx):
    ...
    with conn.cd(PROJECT_PATH):  #mientras este en el with se va a donde tu le mandes
